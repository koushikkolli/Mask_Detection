...
PATH_TO_CKPT = TRAINED_MODEL_DIR + '/frozen_inference_graph1.pb'
PATH_TO_LABELS1 = TRAINED_MODEL_DIR + '/labelmap.pbtxt'
PATH_TO_CKPT1 = TRAINED_MODEL_DIR + '/ssd_lite.pb'
NUM_CLASSES = 2
NUM_CLASSES1 = 90
...
categories1 = label_map_util.convert_label_map_to_categories(
    label_map1, max_num_classes=NUM_CLASSES1, use_display_name=True)
...
def draw_box_on_image1(num_hands_detect, score_thresh, scores, boxes, classes, im_width, im_height, image_np,
    ...
    for i in range(num_hands_detect):
        ...
        if (scores[i] > score_thresh):
            ...
            p1 = (int(left), int(top))
            p2 = (int(right), int(bottom))
            mid1 = (int(left), int((int(top)+int(bottom))/2))
            mid2 = (int(right), int((int(top) + int(bottom)) / 2))
            if i ==0:
                mid = (mid1, mid2)
            #dist = distance_to_camera(avg_width, focalLength, int(right - left))
            ...
            cv2.putText(image_np, 'person ' + str(i) + ': ' + "1", (int(left), int(top) - 5),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            ...
            if i >0 and (mid is not None):
                a = alertcheck.drawboxtosafeline(image_np, mid1, mid2, mid,"lr", im_width, im_height)
            else:
                a =1
        if hand_cnt == 0:
            b = 0
